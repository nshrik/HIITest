...
//Check for decimal with two precision
jQuery.validator.addMethod("decimalWithTwoPrec", function (value, element) {
    return this.optional(element) || /^[0-9]+(\.[0-9]{1,2})?$/i.test(value);
}, "Decimals with two precision");

//jQuery.validator.addMethod("dateInMDY", function (value, element) {
...
jQuery.validator.addMethod("dateMDY", function (value, element) {
                ...
                var re = /^(0[1-9]|1[012])[-/.](0[1-9]|[12][0-9]|3[01])[-/.](19|20)\d\d$/;
                 if (re.test(value)) {
                    var adata = value.split('/');
                    var mm = parseInt(adata[0], 10); // is month
                    var dd = parseInt(adata[1], 10); // is day
                    var yyyy = parseInt(adata[2], 10); // is year
                    var xdata = new Date(yyyy, mm - 1, dd);
                        if ((xdata.getFullYear() == yyyy) && (xdata.getMonth() == mm - 1) && (xdata.getDate() == dd))
                            check = true;
                        else
...
jQuery.validator.addMethod("requiredDDL", function (value, element) {
  var check = false;
