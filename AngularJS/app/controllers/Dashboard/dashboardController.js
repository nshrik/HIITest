//app.
"use strict";
app.controller('dashboardController', function ($rootScope, $scope, $q, ngAuthSettings, $interval, uiGridExporterConstants, authService, UtilService, dashboardHelper, dashboardService, shareableService, modal) {
		...
		$(".autosuggest").autocomplete({
			source: function (request, response) {
				if (request.term.length >= 1) {
					var query = {
						searchParams: request.term,
						currentUserRoleCode: vm.hdnCurrent.UserInfo.CurrentUserRoleCode
					};

					//get the data
					shareableService.providerSearch(query).then(function (result) {
						//process the response
						if (result == null || result.length == 0 || result.data.ProviderSearchResult == null
							|| result.data.ProviderSearchResult == undefined || result.data.ProviderSearchResult.length == 0) {
							response([{ label: 'No results found.', val: -1 }]);
						}
						else {
							response($.map(result.data.ProviderSearchResult, function (item) {
								return {
